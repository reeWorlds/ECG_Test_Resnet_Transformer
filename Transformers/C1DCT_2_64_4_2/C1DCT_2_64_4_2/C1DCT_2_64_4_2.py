import C1DCT
from C1DCT import Conv1DCompactTransformer
...
def readTrainData():
	...
	trainData = tf.reshape(trainData, (trainData.shape[0], trainData.shape[1], 1))
...
def readValidData():
	...
	validData = tf.reshape(validData, (validData.shape[0], validData.shape[1], 1))
...
def getScedule(initial_learning_rate, datasetLen, minibatch, epochN, num_waves = g_waves, warmup_epochs = 4):
...
def plot_lr_schedule(path):
	total_steps = (68062 + g_batch_size - 1) // g_batch_size * g_epochs
	lr_schedule = getScedule(1e-3, 68062, g_batch_size, g_epochs)
...
class CustomLearningRateSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
	def __init__(self, initial_learning_rate, datasetLen, minibatch, epochN, num_waves=g_waves, warmup_epochs=4):
...
def getModel(custom_lr_schedule, decay):
	...
	outputs = Conv1DCompactTransformer(g_n_blocks, g_n_embd, g_n_heads, g_scale)(inputs)
...
def trainNN(attempt, initial_learning_rate, weight_decay):
	...
	checkpoint_callback = ModelCheckpoint(filepath=f'../../../models/C1DCT_2_64_4_2/best_attempt{attempt}.h5', 
	...
	with open(f'../../../models/C1DCT_2_64_4_2/history_attempt{attempt}.csv', 'w') as f:
...
def trainLogReg(attempt):
	...
	custom_objects = { "CustomAdamW": CustomAdamW, "CustomLearningRateSchedule": CustomLearningRateSchedule,
				   "Conv1DCompactTransformer": Conv1DCompactTransformer }
	NNmodel = keras.models.load_model(f'../../../models/C1DCT_2_64_4_2/best_attempt{attempt}.h5', custom_objects=custom_objects)
	...
	with open(f'../../../models/C1DCT_2_64_4_2/logreg_attempt{attempt}.pkl', 'wb') as file:
...
def testF1(attempt):
	custom_objects = { "CustomAdamW": CustomAdamW, "CustomLearningRateSchedule": CustomLearningRateSchedule,
				  "Conv1DCompactTransformer": Conv1DCompactTransformer }
	model = keras.models.load_model(f'../../../models/C1DCT_2_64_4_2/best_attempt{attempt}.h5', custom_objects=custom_objects)
	...
	with open(f'../../../models/C1DCT_2_64_4_2/logreg_attempt{attempt}.pkl', 'rb') as file:
...
def trainLogRegFull(listAttempts):
	...
	custom_objects = { "CustomAdamW": CustomAdamW, "CustomLearningRateSchedule": CustomLearningRateSchedule,
				  "Conv1DCompactTransformer": Conv1DCompactTransformer }
	NNmodels = [keras.models.load_model(f'../../../models/C1DCT_2_64_4_2/best_attempt{x}.h5', custom_objects=custom_objects) for x in listAttempts]
	...
	with open(f'../../../models/C1DCT_2_64_4_2/logreg_full.pkl', 'wb') as file:
		pickle.dump(logreg, file)
...
def testF1Full(listAttempts):
	...
	with open(f'../../../models/C1DCT_2_64_4_2/logreg_full.pkl', 'rb') as file:
		logreg = pickle.load(file)
...
if __name__ == "__main__":
	# testGPU()
	# plot_lr_schedule("../../../models/C1DCT_2_64_4_2/schedule.png")

	#for attempt in range(1, 4):
	#	trainNN(attempt, 1e-3, 1e-1)
	#	plotTrainValidCurve(f"../../../models/C1DCT_2_64_4_2/history_attempt{attempt}.csv",
	#				 f"../../../models/C1DCT_2_64_4_2/history_attempt{attempt}.png")
