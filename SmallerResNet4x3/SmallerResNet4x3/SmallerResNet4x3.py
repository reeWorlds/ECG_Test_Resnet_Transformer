...
def getModel(custom_lr_schedule, decay):
	...
	x = tf.keras.layers.Conv1D(74, 1, padding='same', activation='relu')(x)
	x = tf.keras.layers.BatchNormalization()(x)
	x = tf.keras.layers.Conv1D(74, 3, padding='same', activation='relu')(x)
	x = tf.keras.layers.BatchNormalization()(x)
...
def trainNN(attempt, initial_learning_rate, weight_decay):
	...
	checkpoint_callback = ModelCheckpoint(filepath=f'../../models/SmallerResNet4x3/best_attempt{attempt}.h5', 
	...
	with open(f'../../models/SmallerResNet4x3/history_attempt{attempt}.csv', 'w') as f:
...
def trainLogReg(attempt):
	...
	NNmodel = keras.models.load_model(f'../../models/SmallerResNet4x3/best_attempt{attempt}.h5', custom_objects=custom_objects)
	...
	with open(f'../../models/SmallerResNet4x3/logreg_attempt{attempt}.pkl', 'wb') as file:
...
def testF1(attempt):
	custom_objects = { "CustomAdamW": CustomAdamW, "CustomLearningRateSchedule": CustomLearningRateSchedule }
	model = keras.models.load_model(f'../../models/SmallerResNet4x3/best_attempt{attempt}.h5', custom_objects=custom_objects)
	...
	with open(f'../../models/SmallerResNet4x3/logreg_attempt{attempt}.pkl', 'rb') as file:
...
def trainLogRegFull(listAttempts):
	...
	custom_objects = { "CustomAdamW": CustomAdamW, "CustomLearningRateSchedule": CustomLearningRateSchedule }
	NNmodels = [keras.models.load_model(f'../../models/SmallerResNet4x3/best_attempt{x}.h5', custom_objects=custom_objects) for x in listAttempts]
	...
	with open(f'../../models/SmallerResNet4x3/logreg_full.pkl', 'wb') as file:
		pickle.dump(logreg, file)
...
def testF1Full(listAttempts):
	...
	with open(f'../../models/SmallerResNet4x3/logreg_full.pkl', 'rb') as file:
		logreg = pickle.load(file)
...
if __name__ == "__main__":
	...
	#plot_lr_schedule("../../models/SmallerResNet4x3/schedule.png")
	
	#for attempt in range(1, 4):
	#	trainNN(attempt, 1e-3, 1e-1)
	#	plotTrainValidCurve(f"../../models/SmallerResNet4x3/history_attempt{attempt}.csv",
	#				 f"../../models/SmallerResNet4x3/history_attempt{attempt}.png")
