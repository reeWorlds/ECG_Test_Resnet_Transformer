...
def getScedule(initial_learning_rate, datasetLen, minibatch, epochN, num_waves = 8, warmup_epochs = 4):
...
def plot_lr_schedule(path):
	total_steps = (68062 + 256 - 1) // 256 * 260
	lr_schedule = getScedule(1e-3, 68062, 256, 260)
...
class CustomLearningRateSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
	def __init__(self, initial_learning_rate, datasetLen, minibatch, epochN, num_waves=8, warmup_epochs=4):
...
def getModel(custom_lr_schedule, decay):
	...
	x = tf.keras.layers.Conv1D(56, 5, padding='same', activation='relu')(x)
	x = tf.keras.layers.BatchNormalization()(x)
	x = tf.keras.layers.Conv1D(56, 5, padding='same', activation='relu')(x)
	x = tf.keras.layers.BatchNormalization()(x)
	x = tf.keras.layers.Add()([x, shortcut])
	...
	# Residual block 4
	shortcut = tf.keras.layers.Conv1D(72, 1)(x)
	shortcut = tf.keras.layers.BatchNormalization()(shortcut)

	x = tf.keras.layers.Conv1D(72, 3, padding='same', activation='relu')(x)
	x = tf.keras.layers.BatchNormalization()(x)
	x = tf.keras.layers.Conv1D(72, 3, padding='same', activation='relu')(x)
	x = tf.keras.layers.BatchNormalization()(x)
	x = tf.keras.layers.Add()([x, shortcut])
...
def trainNN(attempt, initial_learning_rate, weight_decay):
	...
	checkpoint_callback = ModelCheckpoint(filepath=f'../../models/SmallerResNet4x2/best_attempt{attempt}.h5', 
	...
	with open(f'../../models/SmallerResNet4x2/history_attempt{attempt}.csv', 'w') as f:
...
def trainLogReg(attempt):
	...
	NNmodel = keras.models.load_model(f'../../models/SmallerResNet4x2/best_attempt{attempt}.h5', custom_objects=custom_objects)
	...
	with open(f'../../models/SmallerResNet4x2/logreg_attempt{attempt}.pkl', 'wb') as file:
...
def testF1(attempt):
	custom_objects = { "CustomAdamW": CustomAdamW, "CustomLearningRateSchedule": CustomLearningRateSchedule }
	model = keras.models.load_model(f'../../models/SmallerResNet4x2/best_attempt{attempt}.h5', custom_objects=custom_objects)
	...
	with open(f'../../models/SmallerResNet4x2/logreg_attempt{attempt}.pkl', 'rb') as file:
...
def trainLogRegFull(listAttempts):
	...
	custom_objects = { "CustomAdamW": CustomAdamW, "CustomLearningRateSchedule": CustomLearningRateSchedule }
	NNmodels = [keras.models.load_model(f'../../models/SmallerResNet4x2/best_attempt{x}.h5', custom_objects=custom_objects) for x in listAttempts]
	...
	with open(f'../../models/SmallerResNet4x2/logreg_full.pkl', 'wb') as file:
		pickle.dump(logreg, file)
...
def testF1Full(listAttempts):
	...
	with open(f'../../models/SmallerResNet4x2/logreg_full.pkl', 'rb') as file:
		logreg = pickle.load(file)
